...
The current foundation has no task transport or automatic skill binding. A Markdown skill alone cannot access an MCP server's private in-memory registry. If the host exposes only knowledge MCP tools, report that AgentTaskContext is unavailable in this session. Do not claim creation, updates, reference association, or a runtime `blocked` status that did not occur. Ask whether to continue explicitly without task tracking or wait for a compatible host; do not silently downgrade LEVEL 2+ to LEVEL 1. Do not implement a bridge, new tools, persistence, or a substitute task type as a workaround.
