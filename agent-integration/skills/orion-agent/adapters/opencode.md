...
- Global `mcp.orion` provides technical access to `orion_ping`, `orion_search`, `orion_read`, `orion_context`, `orion_related`, and `orion_remember`.
...
- OpenCode Markdown skills still cannot receive JavaScript runtime objects. Optional `dist/opencode/orion-agent-runtime-plugin.mjs` is an in-process OpenCode host adapter: it requires host-supplied `ORION_VAULT_ROOT`, owns one `AgentRuntimeHost` per OpenCode session and exposes only task creation, sequential workflow commands, and explicit session disposal. It is not MCP, does not expose runtime internals or vault roots, and uses no fallback task store.
