...
- OpenCode has no runtime-injection hook for Markdown skills. It cannot receive `AgentRuntimeHost` in this adapter; only a host that explicitly injects `AgentRuntime` may expose `knowledge` and `tasks` in-process. This does not add an MCP task transport or fallback store.
