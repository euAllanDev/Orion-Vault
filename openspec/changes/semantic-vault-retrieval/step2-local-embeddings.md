...
- por isso, a rodada pratica seguinte passa a incluir uma variante experimental `sentence-tight` de chunking no benchmark para `noop` e `token-hash`, preservando o mesmo reranking. A pergunta central deixa de ser “qual embedder promover” e passa a ser “uma unidade de recuperacao mais granular melhora a nota canônica sem subir demais o custo local?”
- a rodada curada ampliada respondeu essa pergunta com mais sinal: mesmo com notas longas e queries pensadas para forçar seções concorrentes, `sentence-tight` continuou empatado com o baseline equivalente em acerto de nota canônica. Em contrapartida, o corpus ampliado expôs um problema melhor para atacar: notas estruturais de `knowledge` passaram a ganhar peso excessivo em queries abstratas de outros domínios, sugerindo que o próximo refinamento útil está mais em conter generalização heurística do que em reduzir ainda mais o tamanho do chunk
- o refinamento heurístico seguinte confirmou essa leitura: ao conter a generalização estrutural excessiva e ampliar o peso relativo do score combinado chunk+nota, o sistema eliminou os principais falsos positivos cross-domain do corpus ampliado sem depender de mudar provider ou promover `sentence-tight`. A lacuna remanescente ficou intra-domínio em arquitetura, onde uma nota de glossário ainda pode vencer a nota de runtime em uma query conceitual abstrata. Isso sugere que a próxima hipótese forte deve mirar “referência vs execução dentro do mesmo domínio” e não “vetor vs lexical” nem “chunk menor vs chunk maior”
